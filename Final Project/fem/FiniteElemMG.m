...
%   argument.
function [u,node,elem]= FiniteElemMG(node,elem,f,J,varargin)
    %% Initial Setup
    %
    ...
    M = accumarray([elem(:,1);elem(:,2);elem(:,3)],[area;area;area]/3,[N(i+1),1]);
    Ma=spdiags(M,0,N(i+1),N(i+1));
    As{J}=Asfull(freeNode,freeNode);%-Ma(freeNode,freeNode); %clear Asfull;
    ...
    %
    g_D=varargin{1}; u = zeros(N(J+1),1);
    ...
    if(length(varargin)>3 && strcmpi(varargin{4},'pcg'))
        ...
        [u(freeNode),k,res]=PCGvCycleAlg(r,u(freeNode),J,As,...
                            Res,Pro,Smoothers,tol,itrmax,3);
    else
        %%% Multi-Grid Method
        %
        ...
        e=FEMvCycleAlg(r,J,As,Res,Pro,Smoothers,3);
