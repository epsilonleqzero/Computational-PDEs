function [eta] = estimateresidualheatb(node,elem,u,pde,uold,dt,bdFlag)
...
%   For time adaptivity.
%   Unused for now
ut=(un-un1); Duh = recovery(node,elem,Du,area);
...
recoveryresidual=area.*((dgu)); edgeJumps=(edgeJump)/2;
elemTimeResidual=((ut/dt)-elemResidual).*(area);
%% Residual Error Estimator
%
eta = (abs(edgeJumps).*(area.^(1/2))+ ...
        ((epsilonresidual.^2))+(elemTimeResidual.^2)+ ...
        (recoveryresidual.^2)).^(1/2);